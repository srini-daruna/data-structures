...
public class Tree
{
    ...
    {
	Tree root = this;
	// Case 1 if there are no children to the node
	if(node.leftNode != null && node.rightNode == null)
	{
    ...
    public Tree findInOrderPredecessor(int data)
    {
	// This is sample code and assumes that data mentioned is present in the tree. 
	Tree root = this;
	Tree node = root.getNode(data);
	...
	if(node.leftNode == null)
	{
	    ...
	    while(root != null)
	    {
		if(root.leftNode.findMin().data == data)
		{
		    break;
		}
	    ...
	    return node.leftNode.findMax();
	}
    }
    
    public Tree findInOrderSuccessor(int data)
    {
	// This is sample code and assumes that data mentioned is present in the tree. 
	Tree root = this;
	Tree node = root.getNode(data);
	...
	if(node.rightNode == null)
	    ...
	    while(root != null)
	    {
		if(root.leftNode.findMax().data == data)
		{
		    break;
		}
    ...
    public Tree getNode(int data)
    {
	Tree root = this;
	
	if(root.data < data)
	{	
	    return root.rightNode.getNode(data);
	}
	else if(root.data > data){
	    return root.leftNode.getNode(data);
	}
	else if(root.data == data)
	    return root;
	else
